	...
	<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
		...
		<div class="min-w-0">
			<div class="flex items-center gap-2 mb-1">
				...
				<p class="text-xs text-(--color-text-subtle) uppercase tracking-wider">In-flight</p>
			</div>
		...
		<!-- Req/s -->
		<div class="min-w-0">
			<p class="text-xs text-(--color-text-subtle) uppercase tracking-wider mb-1">Req/s</p>
			<p class="text-xl font-bold text-(--color-text) tabular-nums">
		...
		<!-- Tokens/s -->
		<div class="min-w-0">
			<p class="text-xs text-(--color-text-subtle) uppercase tracking-wider mb-1">Tokens/s</p>
		...
		<!-- Errors/min -->
		<div class="min-w-0">
			<p class="text-xs text-(--color-text-subtle) uppercase tracking-wider mb-1">Errors/min</p>
			<p class="text-xl font-bold {live.errorsPerMin > 0 ? 'text-red-400' : 'text-(--color-text)'} tabular-nums">
		...
		<!-- Fleet -->
		<div class="min-w-0">
			<p class="text-xs text-(--color-text-subtle) uppercase tracking-wider mb-1">Fleet</p>
			<p class="text-xl font-bold text-(--color-text) tabular-nums">
			...
			<div class="mt-1 h-7 flex items-center">
				...
				{:else}
					<span class="text-xs text-(--color-text-subtle)">backends</span>
				{/if}
			</div>
		</div>
	</div>
	{#if cold}
		<p class="mt-3 text-xs text-(--color-text-subtle) text-center">Waiting for traffic</p>
	{/if}
</div>
