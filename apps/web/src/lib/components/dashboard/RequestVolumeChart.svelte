...
<div class="card p-4" data-testid="request-volume">
	<div class="flex items-center justify-between mb-4">
		<h2 class="text-sm font-medium text-(--color-text-muted)">Request volume · {windowLabel}</h2>
		<div class="flex items-center gap-3 text-xs text-(--color-text-subtle)">
			<span class="flex items-center gap-1">
				<span class="w-2.5 h-2.5 rounded-sm bg-(--color-brand) opacity-70"></span> requests
			</span>
			<span class="flex items-center gap-1">
				<span class="w-2.5 h-2.5 rounded-sm bg-(--color-error)"></span> errors
			</span>
		</div>
	</div>
	<div class="pb-4">
